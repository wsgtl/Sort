...
export class MD5Util {
    ...
    /**
     * 计算对象的MD5值（先序列化为JSON字符串）
     */
    public static hashObject(obj: any): string {
        if (obj === null || obj === undefined) {
            return this.hash('');
        }
        const str = typeof obj === 'string' ? obj : JSON.stringify(this.sortObject(obj));
     ...
     * 生成带时间戳的签名
     */
    public static generateSign(data: any, key: string, timestamp: number = Date.now()): string {
        // 对对象进行排序以确保签名一致性
        const sortedData = this.sortObject(data);
        const signString = `${JSON.stringify(sortedData)}${key}${timestamp}`;
     ...
     * 验证签名
     */
    public static verifySign(data: any, key: string, timestamp: number, sign: string): boolean {
        const expectedSign = this.generateSign(data, key, timestamp);
     ...
     * MD5算法实现
     */
    private static hexMD5(str: string): string {
        return this.md5(str);
    }
     ...
     * MD5核心算法
     */
    private static md5(s: string): string {
